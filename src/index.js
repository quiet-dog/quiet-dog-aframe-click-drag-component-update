...
const { unproject } = (function unprojectFunction() {
    ...
    return {
        ...
        unproject(THREE, vector, camera) {
            ...
            vector.applyMatrix4(matrix.copy(threeCamera.projectionMatrix).invert());
...
const { selectItem } = (function selectItemFunction() {
    ...
    return {
        selectItem(THREE, selector, camera, clientX, clientY) {
            ...
            // console.log((camera.components.camera.camera as THREE.Camera).getWorldDirection(directionAsVec3).negate())
            // ====================================================================================================
            plane.setFromNormalAndCoplanarPoint(
                // camera.components.camera.camera.getWorldDirection().clone().negate(),
                (camera.components.camera.camera).getWorldDirection(new THREE.Vector3()).clone().negate(),
                point.clone().sub(cameraPosAsVec3)
            );
            // ====================================================================================================
            const depth = plane.constant;
            const offset = point.sub(object.getWorldPosition(new THREE.Vector3()));
...
function dragItem(THREE, element, offset, camera, depth, mouseInfo) {
    ...
    // ===============================================================
    const startCameraRotationInverse = threeCamera.getWorldQuaternion(new THREE.Quaternion()).invert();

    // =================================================================
    const startElementRotation = (element.object3D).getWorldQuaternion(startCameraRotationInverse);
    ...
    function onMouseMove({ clientX, clientY }) {
        ...
        // ==============================================================
        rotationDiff = rotationQuaternion.multiply(threeCamera.getWorldQuaternion(new THREE.Quaternion()));
